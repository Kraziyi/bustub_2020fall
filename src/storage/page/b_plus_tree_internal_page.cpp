...
ValueType B_PLUS_TREE_INTERNAL_PAGE_TYPE::Lookup(const KeyType &key, const KeyComparator &comparator) const {
  auto target = std::lower_bound(array + 1, array + GetSize(), key, [&comparator](const auto &pair, auto k) { return comparator(pair.first, k) < 0; });
  if (target == array + GetSize()) {
                                                ...
                                                BufferPoolManager *buffer_pool_manager) 
{
  ...
  recipient->CopyNFrom(array + start_split_indx, GetMaxSize() - start_split_indx, buffer_pool_manager);
}
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::CopyNFrom(MappingType *items, int size, BufferPoolManager *buffer_pool_manager) {
  ...
  for (int i = 0; i < size; i++) {
    auto page = buffer_pool_manager->FetchPageImpl(ValueAt(i + GetSize()));
    BPlusTreePage *node = reinterpret_cast<BPlusTreePage *>(page->GetData());
    node->SetParentPageId(GetPageId());
    buffer_pool_manager->UnpinPageImpl(page->Get  std::move(array + index + 1, array + GetSize(), array + index);
  IncreaseSize(-1);PageId(), true);
  }
